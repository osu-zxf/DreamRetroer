...
class ReactionNode:
    def __init__(self, parent, pro, template, init_value):
        ...
        self.count = 1
        # one_step_model pro
        self.pro = pro ## from single step mlp probability
    ...
    def add_mol_node(self, mol_node):
        self.children.append(mol_node)
        mol_node.add_parent_reaction_node(self)
    ...
    def select_child(self):
        ...
        # print('succ:', self.succ, ', expand:', check_expansion, 'check_succ:', check_success)
    ...
    def is_template_same_functional_group(self, template_smarts):
        self_rule = self.template
        self_rule = self_rule.replace('F', 'Cl')
        self_rule = self_rule.replace('Br', 'Cl')
        self_rule = self_rule.replace('I', 'Cl')

        target_rule = template_smarts.replace('F', 'Cl')
        target_rule = target_rule.replace('Br', 'Cl')
        target_rule = target_rule.replace('I', 'Cl')

        return (self_rule.split('>>')[1] == target_rule.split('>>')[1]) or (self_rule.split('>>')[0] == target_rule.split('>>')[0])

    def degrade_twins(self):
        '''
        all reaction nodes with the same class and the same parent(twins nodes) degrade
        '''
        for twins in self.parent.children:
            if (twins is not self) and (self.is_template_same_functional_group(twins.template)):
                twins.degrade()

    def degrade(self):
        ## 1. self.value double
        self.Q_value *= 1.2
        ## 2. all grandchild reaction nodes double
        # self.propagate(self.Q_value * 0.5, exclude=None)
    ...
    def print_child(self):
        ...
        for child in self.children:
            succ_list.append(child.succ)
            open_list.append(child.open)
