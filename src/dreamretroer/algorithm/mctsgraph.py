...
import numpy as np
import logging
from dreamretroer.algorithm.search_tree import SearchTree


def mctsgraph(target_mol, target_mol_id, starting_mols, expand_fn, value_fn,
    ...
    if not search_tree.succ:
        for i in range(iterations):
            ...
            while m_next.children_list() != None:
                ...
                # print('reaction Qvalue:', reaction_Qvalue)
                ...
                # print('visit time:', reaction_visit_time)
                ...
                # print('reaction score:', reaction_a)
                ...
                # if m_next == None:
                #     print('reaction succ:', reaction_nodes[np.argmax(reaction_a)].succ, ', invalid:', reaction_nodes[np.argmax(reaction_a)].valid)
                #     reaction_nodes[np.argmax(reaction_a)].print_child()
                # print(m_next.mol)
            ...
            ## if the parent node of m_last and m_next nodes are different, last reaction should be degraded
            # logging.info('test log %d' % i)
            # if ((m_last is not None) and (m_last.parent is not None) and (m_next.parent is not None)):
            #     if (m_last.parent is not m_next.parent):
            #         m_last.parent.degrade_twins()
            # m_last = m_next
            
            # one step model
            # logging.info("expand %s" % m_next.mol)
    ...
    if search_tree.succ:
        ...
        logging.info('product_id:%d,length:%d' % (target_mol_id, solution_route.length))

    # save_file_name = 'results/syn_tree_%d.txt' % (target_mol_id)
    # search_tree.save_to_file(save_file_name)
