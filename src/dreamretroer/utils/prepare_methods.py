...
from dreamretroer.algorithm import mctsgraph
import json
...
# eg_mcts
def prepare_graph_planner(one_step, value_fn, starting_mols, expansion_topk,
                            iterations, viz=False, viz_dir=None):
    expansion_handle = lambda x: one_step.run(x, topk=expansion_topk)

    plan_handle = lambda x, y=0: mctsgraph(
        target_mol=x,
        target_mol_id=y,
