...
class SynRoute:
    ...
    def add_reaction(self, mol, value, template, reactants, cost, idx):
        assert mol in self.mols
    ...
    def viz_route(self, viz_file):
        ...
        while not node_queue.empty():
            ...
            if parent_idx >= 0:
                G.edge(names[parent_idx], names[idx], label=str(self.rule_idxs[parent_idx]))
        ...
        with open(file_name, 'w') as f:
            for i in range(len(self.mols)):
                if self.children[i] is not None:
                    f.write(self.mols[i])
                    f.write('>')
                    f.write(str(self.rule_idxs[i]))
                    f.write('\n')

                    f.write(str(self.rule_idxs[i]))
                    f.write('>')
                    for mol_id in self.children[i]:
                        f.write(self.mols[mol_id])
                        f.write('...')
                    f.write('\n')
